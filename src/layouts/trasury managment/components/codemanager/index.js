...
const ContigencyCodeTable = () => {
  ...
  const handleCreate = async () => {
    try {
      ...
      await api.post("/contigency-code", formData, {
        headers: {
  ...
  function getEventSourceUrl() {
    const token = localStorage.getItem("token");
    return `${process.env.REACT_APP_API_BASE_URL}api/treasury/total-balance?token=${token}`;
  }
  useEffect(() => {
    fetchData();
  ...
  return (
    ...
    >
      ...
      <Dialog open={openCreateDialog} onClose={() => setOpenCreateDialog(false)}>
        ...
        <DialogActions>
          <MDButton onClick={() => setOpenCreateDialog(false)} color="secondary">
            Cancel
          </MDButton>
          <MDButton onClick={handleCreate} color="success">
            Create
          </MDButton>
        </DialogActions>
      </Dialog>
